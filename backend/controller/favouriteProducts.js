...
module.exports.getFavouriteProductes = async (req, res) => {
  ...
  try {
    const favProducts = await FavouriteProducts.findOne({ userId })
      .populate('products');
...
module.exports.addToFavouriteProducts = async (req, res) => {
  ...
  try {
    ...
    const favouriteProducts = await FavouriteProducts
      .findOne({ userId })
      .populate('products');

    const exisitingProduct = favouriteProducts.products
    ...
    const updatedData = await FavouriteProducts
      .findOneAndUpdate({ userId }, updatedFavProducts, {
        new: true,
      }).populate('products');
...
module.exports.removeFromFavouriteProducts = async (req, res) => {
  ...
  try {
    ...
    if(exisitingProduct) {
      ...
      const updatedData = await FavouriteProducts
        .findOneAndUpdate({ userId }, updatedFavProducts, { new: true })
        .populate('products');
